...
REPLICATED_VERSION="2.43.1"
PINNED_DOCKER_VERSION="18.09.2"
...
pullReplicatedImages() {
    docker pull "${REGISTRY_ADDRESS_OVERRIDE:-quay.io}/${REGISTRY_PATH_PREFIX}replicated/replicated:stable-2.43.1"
    docker pull "${REGISTRY_ADDRESS_OVERRIDE:-quay.io}/${REGISTRY_PATH_PREFIX}replicated/replicated-ui:stable-2.43.1"
    if [ -n "$REGISTRY_ADDRESS_OVERRIDE" ] || [ -n "$REGISTRY_PATH_PREFIX" ]; then
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE:-quay.io}/${REGISTRY_PATH_PREFIX}replicated/replicated:stable-2.43.1" "quay.io/replicated/replicated:stable-2.43.1")
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE:-quay.io}/${REGISTRY_PATH_PREFIX}replicated/replicated-ui:stable-2.43.1" "quay.io/replicated/replicated-ui:stable-2.43.1")
    fi
}
...
#######################################
pullOperatorImage() {
    docker pull "${REGISTRY_ADDRESS_OVERRIDE:-quay.io}/${REGISTRY_PATH_PREFIX}replicated/replicated-operator:2.43.1"
    if [ -n "$REGISTRY_ADDRESS_OVERRIDE" ] || [ -n "$REGISTRY_PATH_PREFIX" ]; then
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE:-quay.io}/${REGISTRY_PATH_PREFIX}replicated/replicated-operator:2.43.1" "quay.io/replicated/replicated-operator:2.43.1")
    fi
}
...
tagAndPushOperatorImage()  {
    docker tag \
        "quay.io/replicated/replicated-operator:2.43.1" \
        "${1}/replicated/replicated-operator:2.43.1"
    docker push "${1}/replicated/replicated-operator:2.43.1"
}
...
tag_docker_images() {
    ...
    docker tag "quay.io/replicated/replicated:stable-2.43.1" "quay.io/replicated/replicated:current" 2>/dev/null \
        || docker tag -f "quay.io/replicated/replicated:stable-2.43.1" "quay.io/replicated/replicated:current"
    docker tag "quay.io/replicated/replicated-ui:stable-2.43.1" "quay.io/replicated/replicated-ui:current" 2>/dev/null \
        || docker tag -f "quay.io/replicated/replicated-ui:stable-2.43.1" "quay.io/replicated/replicated-ui:current"
}
...
install_operator() {
    ...
    if [ "$AIRGAP" != "1" ]; then
        ...
        echo -e "${URLGET_CMD} https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.43.1${NC}"
        ${URLGET_CMD} "https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.43.1" > /tmp/operator_install.sh
    fi
